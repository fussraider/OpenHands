...
| **Workflows** | **Issue Resolver** | `openhands/resolver/` | ✅ Complete | Skeleton CLI tool `cmd/resolver/main.go` implemented for resolving issues. |
| **API** | **Public Options (Dynamic)** | `openhands/server/routes/public.py` | ✅ Complete | Dynamic fetching of supported models/agents for `/api/options/*` implemented. |
...
|---|---|---|---|---|
| **Storage** | **Cloud & Webhooks** | `openhands/storage/` | Medium | File and event persistence backends for S3, Google Cloud, and Webhooks are not yet implemented (only local file storage is currently supported). |
| **Workflows** | **Issue Resolver Logic** | `openhands/resolver/` | High | The full automated pipeline logic for cloning repos, running critic evaluations, and creating PRs (only the CLI skeleton is implemented). |
| **Evaluation** | **Critic Module** | `openhands/critic/` | Low | Heuristic evaluation of agent outputs used primarily by the Issue Resolver. |
| **Config** | **Experiment Manager** | `openhands/experiments/` | Low | Dynamic A/B testing and configuration overrides loaded from storage. |
