...
`, owner, repoName, issueTitle, issueBody, owner, repoName, *issueNumber)
	...
	// Branch management and PR creation using standard git commands in the runtime
	branchName := fmt.Sprintf("fix-issue-%d", *issueNumber)

	slog.Info("Agent finished, creating commit and pushing branch", "branch", branchName)

	// Check if there are changes
	statusOut, _, err := rt.Execute(ctx, fmt.Sprintf("cd %s && git status --porcelain", repoName))
	if err != nil {
		slog.Error("Failed to check git status", "error", err)
	} else if strings.TrimSpace(statusOut) != "" {
		// Changes exist, commit and push
		commitCmd := fmt.Sprintf("cd %s && git checkout -b %s && git add . && git commit -m 'Fix issue #%d' && git push origin %s", repoName, branchName, *issueNumber, branchName)
		_, _, err := rt.Execute(ctx, commitCmd)
		if err != nil {
			slog.Error("Failed to push changes", "error", err)
		} else {
			slog.Info("Successfully pushed changes to remote")

			// PR creation using the GitHub client
			newPR := &github.NewPullRequest{
				Title:               github.String(fmt.Sprintf("Fix issue #%d: %s", *issueNumber, issueTitle)),
				Head:                github.String(branchName),
				Base:                github.String("main"),
				Body:                github.String(fmt.Sprintf("This PR fixes issue #%d. \n\nGenerated by OpenHands Resolver.", *issueNumber)),
			...
			if err != nil {
				...
				slog.Info("Successfully created Pull Request", "url", pr.GetHTMLURL())

				// Output resolver format mimicking Python's visualize_resolver_output.py
				fmt.Printf("\n=== RESOLVER OUTPUT ===\n")
		...
		slog.Info("No changes were made by the agent.")
		fmt.Printf("\n=== RESOLVER OUTPUT ===\n")
		fmt.Printf("Issue: #%d - %s\n", *issueNumber, issueTitle)
		fmt.Printf("Status: NO_CHANGES\n")
		fmt.Printf("=======================\n")
	}
}
